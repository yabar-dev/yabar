...
CFLAGS += -std=c99 -Iinclude -pedantic -Wextra -Wall -Os `pkg-config --cflags pango pangocairo libconfig gdk-pixbuf-2.0 alsa`
LDLIBS += -lxcb -lpthread -lxcb-randr -lxcb-ewmh `pkg-config --libs pango pangocairo libconfig gdk-pixbuf-2.0 alsa`
PROGRAM := yabar
PREFIX ?= /usr
